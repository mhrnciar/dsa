...
void set(unsigned short pos, unsigned short value){
    *(unsigned short *) (memory + pos) = value;
}

unsigned short get(unsigned short pos){
...
void *memory_alloc(int size){
    ...
    if(diff == 0){
        ...
        if(p_foot < get(0) - sizeof(short) && get(p_foot + sizeof(short)) % 2 == 0){
            if(prev == 1)
                set(sizeof(short), p_foot + sizeof(short));
            else
                set(prev + 2 * sizeof(short), p_foot + sizeof(short));
            
            if(next > 1)
                set(next + sizeof(short), p_foot + sizeof(short));
        }
        
        else if(p_foot < get(0) - sizeof(short) && get(p_foot + sizeof(short)) % 2 == 1){
            if(prev == 1 && next > 1){
                ...
                set(prev + 2 * sizeof(short), 1);
            else if(prev > 1 && next > 1){
                set(prev + 2 * sizeof(short), next);
                set(next + sizeof(short), prev);
            }
        }
        ...
        for(int i = p_head + sizeof(short); i < p_foot; i += sizeof(short)){
            set(i, 0);
        }
    ...
    // Ak sa blok zmesti do volneho bloku a da sa rozdelit
        ...
        if(foot < get(0) - sizeof(short)){
            ...
            if(prev == 1)
                ...
                set(foot + 2 * sizeof(short), prev);
                set(prev + 2 * sizeof(short), foot + sizeof(short));
            }
            
            if(next > 1){
                set(foot + 3 * sizeof(short), next);
                set(next + sizeof(short), foot + sizeof(short));
            }
        }
        ...
        for(int i = head + sizeof(short); i < foot; i += sizeof(short)){
            set(i, 0);
        }
...
int memory_free(void *valid_ptr){
    ...
    // Ak sa niekde pred nim nachadza volny blok
    else{
...
int memory_check(void *ptr){
    ...
    printf("Smerník ukazuje na nealokovaný blok\n");
    return 0;
}
